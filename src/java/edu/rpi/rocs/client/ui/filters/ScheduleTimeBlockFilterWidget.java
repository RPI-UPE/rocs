...
public class ScheduleTimeBlockFilterWidget extends SimplePanel {
	protected transient TimeSchedulerFilter owner = null;
	
	public void setOwner(TimeSchedulerFilter owner) {
		this.owner = owner;
	}
	...
	public ScheduleTimeBlockFilterWidget() {
		...
		for(int i=0;i<2*(22-8);i++) {
			if(i%2==0) {
				...
				if(j>11) am = false;
				if(j>12) j -= 12;
				theTable.setRowHeader(i, Integer.toString(j)+":00 "+(am?"AM":"PM"));
			}
			else {
